...
class App extends Component {
  ...
  setTime = () => {
    ...
    this.setState(prevState => ({
      timeValue: prevState.timeValue - 1,
    }))
  ...
  startTime = () => {
    this.intervalTime = setInterval(this.setTime, 1000)
  }

  clickTab = tabValue => {
    this.setState({clickTabId: tabValue})
  }

  getFilteredImages = () => {
    const {clickTabId} = this.state
    const filteredImagesList = imagesList.filter(
      eachImageList => eachImageList.category === clickTabId,
    )
    return filteredImagesList
  }

  getRandomDisplayImage = () => {
    const {thumbnailImageId, displayImageId} = this.state
    const randomNum = Math.ceil(Math.random() * (imagesList.length - 1))
    const NewDisplayImage = imagesList[randomNum].imageUrl
    if (thumbnailImageId === displayImageId) {
      this.setState(prevState => ({
        scoreValue: prevState.scoreValue + 1,
        displayImage: NewDisplayImage,
        displayImageId: imagesList[randomNum].id,
      }))
    } else {
      this.stopTime()
    ...
    return (
      <div className="game-over-container">
        ...
        <div className="final-result-details">
          ...
          <p className="final-score">{scoreValue}</p>
          <button
            type="button"
            className="play-again-container"
            onClick={this.onClickPlayAgain}
          >
            <div className="play-gain-btn">
              <img
  ...
  getHeader = () => {
    const {scoreValue, timeValue} = this.state
    return (
          ...
          <nav className="nav-container">
            ...
            <ul className="header-list-container">
              <li className="score-container">
                <p className="score">
                  Score:<span className="value">{scoreValue}</span>
                </p>
              </li>
              <li className="timer-container">
                <img
                  ...
                  alt="timer"
                />
                <p className="time">{timeValue} sec</p>
              </li>
            </ul>
          </nav>
        </ul>
  ...
  render() {
    ...
    const {displayImage, clickTabId} = this.state
        ...
        {startGame ? (
          <div className="app-container">
            ...
            <div className="image-container">
              <img src={displayImage} className="img" alt="match" />
            </div>
            <ul className="tabs-list-container">
              {tabsList.map(eachTab => (
                <TabItem
                  ...
                  isActive={clickTabId === eachTab.tabId}
                />
              ))}
            </ul>
            <ul className="match-images-list">
              {filteredImagesList.map(eachList => (
                <ImageItem
                  key={eachList.id}
                  imagesListDetails={eachList}
